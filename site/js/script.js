  ...
  $(function () {
    ...
    function initLightGalleryItem(itemToInit, addClass) {
      ...
      if (!isNoviBuilder) {
        if (
          itemToInit.length &&
          itemToInit.attr("href").split("//").length > 1
        ) {
    ...
    function initMaps() {
      ...
      $.getScript(
        ...
        function () {
          ...
          for (var i = 0; i < plugins.maps.length; i++) {
            var zoom =
              parseInt(plugins.maps[i].getAttribute("data-zoom"), 5) || 6;
    ...
    if (plugins.search.length || plugins.searchResults) {
      ...
      var defaultTemplate =
        '<h4 class="search-title"><a target="_top" href="#{href}" class="search-link">#{title}</a></h4>' +
        "<p>...#{token}...</p>";
    ...
    if (plugins.counter.length) {
      for (var i = 0; i < plugins.counter.length; i++) {
        ...
        $document.on(
          "scroll",
          (function ($counterNotAnimated) {
            return function (event) {
              if (
                !$counterNotAnimated.hasClass("animated") &&
                isScrolledIntoView($counterNotAnimated)
              ) {
